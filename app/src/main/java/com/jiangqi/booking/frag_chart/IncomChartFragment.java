package com.jiangqi.booking.frag_chart;
...
//        获取这个月每天的支出总金额，
        List<BarChartItemBean> list = DBManager.getSumMoneyOneDayInMonth(year, month, kind, LoginActivity.getmUsername());
        if (list.size() == 0) {
...
//        获取本月收入最高的一天为多少，将他设定为y轴的最大值
        float maxMoney = DBManager.getMaxMoneyOneDayInMonth(year, month, kind,LoginActivity.getmUsername());
